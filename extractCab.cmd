...
if "!cabId:*-%arch%=!"=="" (
  ...
  findstr /i "Package_for_SafeOSDU" "tmp\%cabId%\update.mum" && set _type=SafeOS
  :: if "!_type!"=="" findstr /i "WinPE" "tmp\%cabId%\update.mum" && (findstr /i "Edition""" "tmp\%cabId%\update.mum" || set _type=WinPE)
  if "!_type!"=="" findstr /i "Package_for_RollupFix" "tmp\%cabId%\update.mum" && set _type=LCU
  if "!_type!"=="" for %%i in (
