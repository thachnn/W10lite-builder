...
if not "%cabId:-x64=%"=="%cabId%" (set arch=x64) else if "%cabId:-arm64=%"=="%cabId%" (set arch=x86) else (set arch=arm64)

:: rename SSU dir
if /i not "%cabId:~0,2%"=="KB" (
  set _kb=
  for /f "tokens=3 delims== " %%a in ('findstr /ri /c:"<package  *identifier=" "tmp\%cabId%\update.mum"') do set "_kb=%%~a"
  if not "!_kb!"=="" call :renameCabId "!_kb!-%arch%"
)

if not "!cabId:*-%arch%=!"=="" goto :TypeDetected
:: detect update type
set _type=
...
if not "%_type%"=="" call :renameCabId "%cabId%-%_type%"
